...
class GitHubIssueDebugger:
    ...
    def send_issue_to_prometheus(self, issue_data: Dict, config: Dict) -> Dict:
        ...
        """
        print(f"Sending issue to Prometheus for debug analysis...")
        ...
        if response.status_code == 200:
            ...
            raise Exception(f"Prometheus processing failed: {response.status_code} - {response.text}")
...
def main():
    ...
    parser.add_argument(
        "--push-to-remote",
        action="store_true",
        help="Push fix to remote branch"
    )
    
    parser.add_argument(
        "--dockerfile-content",
        help="Dockerfile content (for specifying container environment)"
    )
    
    parser.add_argument(
        "--image-name",
        help="Docker image name (for specifying container environment)"
    )
    ...
    parser.add_argument(
        "--build-commands",
        nargs="+",
        help="List of build commands"
    )
    
    parser.add_argument(
        "--test-commands",
        nargs="+",
        help="List of test commands"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of candidate patches (default: 4)"
    )
    ...
    if result["success"]:
        ...
        print(f"✅ Successfully processed GitHub Issue")
        ...
        if prometheus_result.get("issue_response"):
            print(f"📝 Prometheus analysis result generated")
